...
impl SimulatorRect {
    ...
    pub fn simulate(&mut self) {
        ...
        let color = self.board.get(&self.ant_position).cloned().unwrap_or(0);
        // Rotate by the state of square.
        self.ant_direction = self.states[color] * self.ant_direction;
        //Advance the state of the square by 1, possible wrap to back to 0
        self.board
            .insert(self.ant_position, (color + 1) % self.states.len());
        // Move the ant by its direction.
        self.ant_position += self.ant_direction;
    }
}
